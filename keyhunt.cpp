...
void *thread_process_bsgs_random(void *vargp)	{
	...
	*/
		...
		generate_unique_random_key(base_key, n_range_start, n_range_end);
#if defined(_WIN64) && !defined(__CYGWIN__)
		ReleaseMutex(bsgs_thread);
#else
		pthread_mutex_unlock(&bsgs_thread);
...
void *thread_process_bsgs_dance(void *vargp)	{
	...
	*/
	...
	if(BSGS_CURRENT.IsLower(&n_range_end))	{
		generate_unique_random_key(base_key, BSGS_CURRENT, n_range_end);
...
// Random key tracking implementation
void init_random_tracker() {
	if (FLAGRANDOM) {
...
bool generate_unique_random_key(Int &key, const Int &min_range, const Int &max_range) {
	...
	// If we can't find a unique key after max_attempts, just use the last generated one
	// This prevents infinite loops in case the range is small or heavily used
	mark_key_used(key);
	return true;
}
